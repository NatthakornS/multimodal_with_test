...
class selfsupervised:
    ...
    def tests(self):
        ...
        for i_iter, sample_batched in enumerate(self.dataloaders["test"]):
            ...
            self.test_paired_accuracy += is_paired_accuracy.item() / self.val_len_data
    ...
    def _init_dataloaders_test(self):

        test_filename_list = []
        for file in os.listdir(self.configs["dataset"]):
            if file.endswith(".h5"):
                test_filename_list.append(self.configs["dataset"] + file)

        self.logger.print(
            "Number of files in multifile dataset = {}".format(len(test_filename_list))
        )
